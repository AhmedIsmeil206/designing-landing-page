...
function CompanyFeatureRow({ reversed = false, children }: CompanyFeatureRowProps) {
    return (
        <div css={rowDirectionStyles(reversed)}>
            {typeof children === 'function' ? children({ reversed }) : children}
        </div>
    );
}
...
function CompanyFeatureContent({
    ...
    const defaultAction = (
        <Button style={btnStyle} onClick={() => navigate('/blog')}>
            {ctaLabel}
        </Button>
    );
...
function CompanyFeatureImage({ imageSrc, imageAlt }: { imageSrc: string; imageAlt: string }) {
    return (
        <div css={imageWrap}>
